...
const imageBuffer1 = generateWorkoutDisplay('athlete1', testLogbookResultWithSplits);
...
console.log(`Workout had ${testLogbookResultWithSplits.workout.splits?.length || 0} splits`);

// Test 2: Generate with intervals
const imageBuffer2 = generateWorkoutDisplay('athlete2', testLogbookResultWithIntervals);
...
console.log(`Workout had ${testLogbookResultWithIntervals.workout.intervals?.length || 0} intervals`);

// Test 3: Generate simple workout
const imageBuffer3 = generateWorkoutDisplay('athlete3', testLogbookResultSimple);
...
const imageBuffer4 = generateWorkoutDisplay('athlete4', testLogbookResultManyIntervals);
...
console.log(`Workout had ${testLogbookResultManyIntervals.workout.intervals?.length || 0} intervals`);

// Test 5: Generate long workout over 1 hour
const imageBuffer5 = generateWorkoutDisplay('athlete5', testLogbookResultLongWorkout);
...
console.log(`Long workout: ${testLogbookResultLongWorkout.distance}m in 1:22:06.0 with ${testLogbookResultLongWorkout.workout.splits?.length || 0} splits`);
