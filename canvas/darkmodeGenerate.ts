import { createCanvas, Canvas, CanvasRenderingContext2D } from 'canvas';
import { writeFileSync } from 'fs';

// Define types for better type safety
interface WorkoutRow {
  time: string;
  meter: string;
  pace: string;
  strokeRate: string;
  heartRate: string;
}

interface DarkModeColors {
  background: string;
  surface: string;
  primary: string;
  secondary: string;
  accent: string;
  border: string;
}

// Create canvas
const canvas: Canvas = createCanvas(800, 600);
const ctx: CanvasRenderingContext2D = canvas.getContext('2d');

// Dark Mode Professional color palette
const colors: DarkModeColors = {
  background: '#1a1a1a',
  surface: '#2d2d2d',
  primary: '#ffffff',
  secondary: '#b3b3b3',
  accent: '#28a745',
  border: '#404040'
};

// Fill background
ctx.fillStyle = colors.background;
ctx.fillRect(0, 0, 800, 600);

// Subtle outer border with glow effect
ctx.strokeStyle = colors.border;
ctx.lineWidth = 2;
ctx.shadowColor = colors.border;
ctx.shadowBlur = 4;
ctx.strokeRect(40, 40, 720, 520);
ctx.shadowBlur = 0;

// Title area with premium typography
ctx.fillStyle = colors.primary;
ctx.font = 'bold 36px "SF Pro Display", -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto';
ctx.fillText('8000m', 60, 100);

ctx.fillStyle = colors.secondary;
ctx.font = '20px "SF Pro Display", -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto';
ctx.fillText('September 17, 2025', 60, 130);

// Accent line
ctx.strokeStyle = colors.accent;
ctx.lineWidth = 3;
ctx.shadowColor = colors.accent;
ctx.shadowBlur = 6;
ctx.beginPath();
ctx.moveTo(60, 150);
ctx.lineTo(200, 150);
ctx.stroke();
ctx.shadowBlur = 0;

// Table headers with dark surface
ctx.fillStyle = colors.surface;
ctx.fillRect(60, 170, 680, 50);

ctx.strokeStyle = colors.border;
ctx.lineWidth = 1;
ctx.strokeRect(60, 170, 680, 50);

ctx.fillStyle = colors.accent;
ctx.font = 'bold 14px "SF Pro Display", -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto';
ctx.fillText('TIME', 80, 195);
ctx.fillText('DISTANCE', 200, 195);
ctx.fillText('SPLIT /500M', 340, 195);
ctx.fillText('S/MIN', 480, 195);
ctx.fillText('BPM', 580, 195);

// Table rows data
const rowData: WorkoutRow[] = [
  { time: '36:07.1', meter: '8000', pace: '2:15.4', strokeRate: '19', heartRate: '143' },
  { time: '7:15.7', meter: '1600', pace: '2:16.1', strokeRate: '18', heartRate: '104' },
  { time: '7:13.4', meter: '3200', pace: '2:15.4', strokeRate: '19', heartRate: '149' },
  { time: '7:11.3', meter: '4800', pace: '2:14.7', strokeRate: '19', heartRate: '150' },
  { time: '7:15.4', meter: '6400', pace: '2:16.0', strokeRate: '19', heartRate: '150' },
  { time: '7:11.2', meter: '8000', pace: '2:14.7', strokeRate: '20', heartRate: '163' }
];

// Draw table rows with sophisticated dark styling
let yPos: number = 240;
ctx.font = '16px "SF Pro Display", -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto';

rowData.forEach((row: WorkoutRow, index: number) => {
  // Highlight first row with accent color
  if (index === 0) {
    ctx.fillStyle = colors.accent + '20'; // Semi-transparent green
    ctx.fillRect(60, yPos - 20, 680, 40);

    ctx.strokeStyle = colors.accent;
    ctx.lineWidth = 2;
    ctx.shadowColor = colors.accent;
    ctx.shadowBlur = 8;
    ctx.strokeRect(60, yPos - 20, 680, 40);
    ctx.shadowBlur = 0;
  }

  // Alternate row backgrounds
  if (index > 0 && index % 2 === 1) {
    ctx.fillStyle = colors.surface + '40'; // Semi-transparent surface
    ctx.fillRect(60, yPos - 20, 680, 40);
  }

  // Text with appropriate contrast
  ctx.fillStyle = index === 0 ? colors.accent : colors.primary;

  ctx.fillText(row.time, 80, yPos);
  ctx.fillText(row.meter, 200, yPos);
  ctx.fillText(row.pace, 360, yPos);
  ctx.fillText(row.strokeRate, 500, yPos);
  ctx.fillText(row.heartRate, 600, yPos);

  // Subtle separator lines
  if (index < rowData.length - 1) {
    ctx.strokeStyle = colors.border;
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(60, yPos + 10);
    ctx.lineTo(740, yPos + 10);
    ctx.stroke();
  }

  yPos += 40;
});

// Professional footer with status indicator
ctx.fillStyle = colors.accent;
ctx.beginPath();
ctx.arc(70, 530, 4, 0, Math.PI * 2);
ctx.fill();

ctx.fillStyle = colors.secondary;
ctx.font = '12px "SF Pro Display", -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto';
ctx.fillText('LIVE', 85, 535);
ctx.fillText('Professional Training Dashboard', 150, 535);

// Corner accent elements
ctx.fillStyle = colors.accent;
ctx.fillRect(720, 60, 20, 2);
ctx.fillRect(720, 65, 15, 2);
ctx.fillRect(720, 70, 10, 2);

// Save the image
const buffer: Buffer = canvas.toBuffer('image/png');
writeFileSync('./canvas/images/dark-mode-professional-pm5.png', buffer);

console.log('Dark Mode Professional PM5 display saved as dark-mode-professional-pm5.png');
